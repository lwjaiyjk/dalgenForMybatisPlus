package com.yjk.framework.dalgen.config;
...
public class SqlParser {
    ...
    public SqlParser(String sql) {
        log.debug("The sql parameter is " + sql + ".");
        ...
        try {
            ...
            log.error("Failed to parse sql statement " + sql + " due to exception.", pe);
        }

        log.debug("The parsed statement is " + zst + ".");
    ...
    protected List getParams(ZExp exp) {
        ...
        if (exp instanceof ZQuery) {
            ...
            log.debug("The number of operands for expression \"" + expression + "\" is " + opNum);
            ...
            if (opNum == 1) {
                ...
                log.debug("The left operand is " + ol + " with type " + ol.getClass() + ".");
                log.debug("The right operand is " + or + " with type " + or.getClass() + ".");

                if ((ol instanceof ZConstant) && (or instanceof ZExpression)) {
                    ZConstant cl = (ZConstant) ol;
                    ZExpression cr = (ZExpression) or;

                    log.debug("The type of the left constant is " + cl.getType());
